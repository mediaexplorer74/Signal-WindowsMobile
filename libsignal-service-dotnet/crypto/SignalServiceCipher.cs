...
namespace libsignalservice.crypto
{
    ...
    public class SignalServiceCipher
    {
        ...
        public OutgoingPushMessage Encrypt(SignalProtocolAddress destination, UnidentifiedAccess unidentifiedAccess, byte[] unpaddedMessage)
        {
            ...
            {
                //SealedSessionCipher sessionCipher = new SealedSessionCipher
                //    (
                //    SignalProtocolStore, 
                //    new SignalProtocolAddress(LocalAddress.E164number, 1)
                //    );
        ...
        private Plaintext Decrypt(SignalServiceEnvelope envelope, byte[] ciphertext)
        {
            try
            {
                ...
                {
                    DecryptionResult? results = sealedSessionCipher.Decrypt
                        (CertificateValidator, ciphertext,
                        (long)envelope.Envelope.ServerTimestamp);

                    //paddedMessage = results.Item2;
                    paddedMessage = results.PaddedMessage;

                    //metadata = new Metadata(results.Item1.Name, 
                    //    (int) results.Item1.DeviceId, (long) envelope.Envelope.Timestamp, 
                    //    true);

                    metadata = new Metadata(results.SenderUuid,
                       (int)results.DeviceId, (long)envelope.Envelope.Timestamp,
                       true);


                    sessionVersion = (uint) sealedSessionCipher.GetSessionVersion(
                        new SignalProtocolAddress(metadata.Sender, 
                        (uint) metadata.SenderDevice));
                }
                else
                {
